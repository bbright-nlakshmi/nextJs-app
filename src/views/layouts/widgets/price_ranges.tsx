...
const PriceRanges: NextPage<Props> = ({ priceRanges }) => {
  ...
  const getName = (item: ProductItem): string => {
    ...
    return (
      ...
      itemAny.title ||
      itemAny.productName ||
      `Product ${itemAny.productId || itemAny.id || "Unknown"}`
    );
  };

  const getRating = (item: ProductItem): number =>
    (item as any).rating || (item as any).averageRating || 0;
    ...
    try {
      ...
      if (objCache?.getAllProducts) {
        ...
        if (Array.isArray(cachedProducts))
          fetchedProducts.push(...cachedProducts);
      }
      ...
      const uniqueProducts = fetchedProducts.filter((item, index, self) => {
        ...
        return (
          id &&
          self.findIndex((p) => ((p as any).productId || p.id) === id) === index
        );
      });
  ...
  const getRangeText = (range: number) => {
    ...
    return currentIndex === 0
      ...
      : {
          main: `₹${previousPrice.toLocaleString(
            "en-IN"
  ...
  const getHeaderText = () => {
    ...
    return currentIndex === 0
      ...
      : `Products ₹${previousPrice.toLocaleString(
          "en-IN"
